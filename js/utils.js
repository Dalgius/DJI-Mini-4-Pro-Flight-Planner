...
export function showCustomAlert(message, titleKeyOrText = "alertInfo") {
    const title = _tr(titleKeyOrText); 
    // Assumiamo che 'message' sia o una chiave di traduzione o una stringa già tradotta.
    // Se è una chiave, la tradiamo, altrimenti la usiamo così com'è.
    const displayMessage = _tr(message); // Se 'message' è una chiave, viene tradotta. Se è già testo, _tr la restituisce.

    if (DOM.customAlertMessageEl && DOM.customAlertOverlayEl && DOM.customAlertTitleEl) {
        DOM.customAlertTitleEl.textContent = title;
        DOM.customAlertMessageEl.textContent = displayMessage; 
        DOM.customAlertOverlayEl.style.display = 'flex';
    } else { 
        console.error("Custom alert elements not found for utils.showCustomAlert!");
        alert(title + ": " + displayMessage); 
    }
}
